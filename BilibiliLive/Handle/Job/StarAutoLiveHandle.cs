...
{
	public class StarAutoLiveHandle : IMessageHandle<Group>
	{
		public IRootModel RootModel => new BilibiliRootModel();

		public string Name => "/开启自动开播 [游戏]";

		public string Description => "开启自动开播";
	...
	IDataStorage dataStorage)
		...
		{
			if (message.IsGroupID(Constants.OPGroupID) && message.IsUserID(Constants.OPAdmin) && message.SplitMsg(" ")[0] == "/开启自动开播")
		...
		public async Task HandleAsync(Group message)
		{
			...
			streamConfig.LiveAreas.FirstOrDefault(q => q.AreaName == liveArea).AutoLive = true;

			sendMessage(MessageChainBuilder.Create().Text("(●• ̀ω•́ )✧自动直播开启成功啦~").Build());
