...
namespace BilibiliLive.Manager
{
	public static class EraManager
	{
		...
		/// <returns></returns>
		public static async Task RefreshEraData(Action<List<MessageSegment>> sendMessage)
		{
			var eraTaskConfig = _dataStorage.Load<EraTaskConfig>(Constants.EraFile);
			//获取原神激励计划的url
			using var getEraUrlResponse = await Tool.HttpClient.SendAsync(new(HttpMethod.Get, $"{Constants.GetEraUrl}?topic_id=3219"));
			...
			sendMessage(MessageChainBuilder.Create().Text($"[{eraTitle}]更新成功啦~").Build());
		...
		/// <param name="group"></param>
		/// <returns></returns>
		public static async Task QueryTasks(Action<List<MessageSegment>> sendMessage)
		{
...
{string.Join("\n", liveRes.Data.List[5].AccumulativeCheckPoints.Select(s => $"{startChar}{s.AwardName}({s.Status})"))}";
		...
		/// <returns></returns>
		public static async Task ReceiveDailyEraAward(Action<List<MessageSegment>> sendMessage)
		{
			var accountConfig = _dataStorage.Load<AccountConfig>(Constants.AccountFile);
			var eraConfig = _dataStorage.Load<EraTaskConfig>(Constants.EraFile);
